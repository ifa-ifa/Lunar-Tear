...
#pragma pack(push, 1)
    ...
    struct RawFileEntry { uint32_t pathHash; uint32_t nameOffset; uint32_t arcOffset; uint32_t compressedSize; uint32_t PackSerialisedSize; uint32_t assetsDataSize; uint8_t archiveIndex; uint8_t flags; uint8_t padding[2]; };
...
#pragma pack(pop)
    ...
    static_assert(sizeof(RawBxonHeader) == 20);
    static_assert(sizeof(RawArchiveFileParam) == 16);
    static_assert(sizeof(RawArchiveEntry) == 12);
    static_assert(sizeof(RawFileEntry) == 28);
...
namespace replicant::bxon {
        ...
        uint32_t projectID)
    {
        ...
        for (size_t i = 0; i < param.getArchiveEntries().size(); ++i) {
            ...
            arc_entry_ptr->unknown_1 = entry.offsetScale;
