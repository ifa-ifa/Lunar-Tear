...
namespace replicant::archive {
    ...
    // --- ArcWriter Implementation ---

    std::expected<void, ArcError> ArcWriter::addFile(std::string key, std::vector<char> data) {
        if (key.empty()) {
            return std::unexpected(ArcError{ ArcErrorCode::EmptyInput, "File key cannot be empty." });
        }
        if (data.empty()) {
            return std::unexpected(ArcError{ ArcErrorCode::EmptyInput, "File data cannot be empty." });
        }
    ...
    std::expected<std::vector<char>, ArcError> ArcWriter::build(int compression_level) {
        ...
        for (const auto& pending_entry : m_pending_entries) {
            ...
            if (!compressed_result) {
                // The compression failed, return its error directly.
                return std::unexpected(compressed_result.error());
            }
            std::vector<char>& compressed_data = *compressed_result;

            size_t padding_needed = (ALIGNMENT - (current_offset % ALIGNMENT)) % ALIGNMENT;
            if (padding_needed > 0) {
            ...
            built_entry.offset = current_offset;
